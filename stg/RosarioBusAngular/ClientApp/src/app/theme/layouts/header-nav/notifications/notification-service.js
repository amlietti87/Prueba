"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
require("rxjs/add/observable/fromPromise");
require("rxjs/add/observable/of");
require("rxjs/add/observable/throw");
require("rxjs/add/operator/map");
require("rxjs/add/operator/toPromise");
require("rxjs/add/operator/mergeMap");
require("rxjs/add/operator/catch");
var core_1 = require("@angular/core");
var environment_1 = require("../../../../../environments/environment");
var http_1 = require("@angular/common/http");
var NotificationServiceProxy = /** @class */ (function () {
    function NotificationServiceProxy(http) {
        this.http = http;
        this.jsonParseReviver = undefined;
        this.identityUrl = '';
        this.identityUrl = environment_1.environment.identityUrl + '/Notification';
        this.baseUrl = this.identityUrl;
    }
    NotificationServiceProxy.prototype.getUserNotifications = function (state, maxResultCount, skipCount) {
        var url_ = this.baseUrl + "/GetUserNotifications?";
        return this.http.get(url_);
    };
    NotificationServiceProxy.prototype.setNotificationAsRead = function (input) {
        var url_ = this.baseUrl + "/SetNotificationAsRead?id=" + input;
        return this.http.post(url_, input);
    };
    NotificationServiceProxy = __decorate([
        core_1.Injectable(),
        __metadata("design:paramtypes", [http_1.HttpClient])
    ], NotificationServiceProxy);
    return NotificationServiceProxy;
}());
exports.NotificationServiceProxy = NotificationServiceProxy;
var UserNotification = /** @class */ (function () {
    function UserNotification() {
    }
    return UserNotification;
}());
exports.UserNotification = UserNotification;
var GetNotificationsOutput = /** @class */ (function () {
    function GetNotificationsOutput() {
    }
    return GetNotificationsOutput;
}());
exports.GetNotificationsOutput = GetNotificationsOutput;
//# sourceMappingURL=notification-service.js.map