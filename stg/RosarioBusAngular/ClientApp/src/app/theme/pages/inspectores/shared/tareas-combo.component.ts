...
import { TareaFilter, TareaDto } from '../model/tarea.model';
import { TareaService } from '../tareas/tarea.service';


@Component({
    selector: 'tareas-combo',
    template: `   
    <select #combobox
            [(ngModel)]="value"
    ...
    providers: [
        {
            ...
            useExisting: forwardRef(() => TareasComboComponent),
            multi: true
        }
    ]
})
export class TareasComboComponent extends ComboBoxComponent<TareaDto> implements OnInit {


    constructor(service: TareaService, injector: Injector) {
    ...
    GetFilter(): TareaFilter {
        var filter = new TareaFilter();
